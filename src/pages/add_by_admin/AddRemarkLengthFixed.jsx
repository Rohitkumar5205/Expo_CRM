...
    } catch (e) {
      console.warn("Failed to parse fix lengths from localStorage", e);
    }
    return [
      { id: 1, name: "50", status: "Active" }
    ];
  });
...
    } catch (e) {
      console.warn("Failed to save fix lengths to localStorage", e);
    }
  }, [fixLengths]);
...
      showMessage("Please enter fix length name!");
      return;
    }
...
    );
    if (duplicate) {
      showMessage("A fix length with that name already exists!");
      return;
    }

    if (editingFixLength) {
      setFixLengths((prev) =>
        prev.map((item) => (item.id === editingFixLength.id ? { ...item, name: trimmedName, status: formData.status } : item))
      );
...
      };
      setFixLengths((prev) => [...prev, newItem]);
...
    const itemToEdit = fixLengths.find((item) => item.id === itemId);
    if (itemToEdit) {
      setFormData({
        name: itemToEdit.name,
        status: itemToEdit.status,
      });
      setEditingFixLength(itemToEdit);
      window.scrollTo({ top: 0, behavior: "smooth" });
    }
  };

  const openDeleteModal = (itemId) => {
    const item = fixLengths.find((c) => c.id === itemId);
...
      return;
    }
    setFixLengths((prev) => prev.filter((c) => c.id !== fixLengthToDelete.id));
...
        >
          <div>
            Are you sure you want to delete this fixed length? This action cannot be undone.
          </div>
        </Modal>
      </div>
    </div>
