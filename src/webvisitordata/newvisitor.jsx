                ...
                <div className="grid grid-cols-5 gap-3 mb-4">
                  <div>
                    ...
                    <select value={industrySector} onChange={(e) => setIndustrySector(e.target.value)}>
                      {["Select Here", "AYUSH & Herbal Products", "Health & Wellness"].map((option, index) => (
                        <option key={index} value={option}>{option}</option>
                      ))}
                    </select>
                  </div>
...
const GeneralVisitorForm = () => {
  ...
  const [companyName, setCompanyName] = useState("");
  const [designation, setDesignation] = useState("");
  const [industrySector, setIndustrySector] = useState("Select Here");
  const [country, setCountry] = useState("Select Country");
  const [state, setState] = useState("Select Country first");
  const [city, setCity] = useState("Select State first");

  const genders = ["Select Here", "Male", "Female", "Other"];
  const purposes = ["Business Networking", "Exploring New Products", "Buying Products & Services", "Learning Industry Trends", "Others"];
  const areasOfInterest = ["AYUSH & Herbal Products", "Organic & Natural Products", "Fitness & Wellness Equipment", "Health Supplements", "Hospitals & Healthcare Services", "Agriculture & Organic Farming", "R&D & Innovations", "Others"];

  const registrationOptions = ["Select Here", "4th Organic Expo 2026", "9th International Health and Wellness Expo"];
