...
const AddTarget = () => {
      ...
      setTargets((prev) =>
        prev.map((item) =>
          item.id === editingTarget.id
            ? { ...item, user: formData.user, target: formData.target, status: formData.status }
            : item
        )
      );
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    if (searchText && searchText.trim()) {
      ...
      list = list.filter((c) => c.user.toLowerCase().includes(s) || String(c.target).includes(s));
    }
    ...
    <div className="w-full" style={{ backgroundColor: "#ecf0f5", minHeight: "100vh", padding: "0" }}>
      ...
      <div className="w-full bg-white" style={{ borderBottom: "1px solid #e0e0e0" }}>
        <div className="flex items-center justify-between px-6 py-3">
          <h1 className="text-lg font-normal" style={{ color: "#666" }}>USER TARGET</h1>
        </div>
      </div>
            ...
            <table className="w-full" style={{ borderCollapse: "collapse", width: "100%" }}>
                ...
                <tr style={{ borderBottom: "2px solid #ddd" }}>
                  ...
                  <th className="px-4 py-3 text-sm font-semibold text-left" style={thStyle()}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      User
                      <button onClick={() => toggleSort("user")} style={styles.sortBtn}>
                        {sortBy.key === "user" ? (sortBy.dir === "asc" ? "▲" : "▼") : "↕"}
                      </button>
                    </div>
                  </th>
                  <th className="px-4 py-3 text-sm font-semibold text-left" style={thStyle()}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      Target
                      <button onClick={() => toggleSort("target")} style={styles.sortBtn}>
                        {sortBy.key === "target" ? (sortBy.dir === "asc" ? "▲" : "▼") : "↕"}
                      </button>
                    </div>
                  </th>
              ...
              <tbody>
                {currentPageData.length === 0 ? (
                  <tr>
                    <td colSpan={5} style={{ padding: 24, textAlign: "center", color: "#777" }}>
