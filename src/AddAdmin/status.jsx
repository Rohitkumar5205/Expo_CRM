...
const Status = () => {
  ...
  const handleAddStatus = () => {
    ...
    if (editingStatus) {
      setStatuses((prev) =>
        prev.map((stat) => (stat.id === editingStatus.id ? { ...stat, name: trimmedName, status: formData.status } : stat))
      );
  ...
  const totalPages = Math.max(1, Math.ceil(filteredAndSortedStatuses.length / rowsPerPage));
  useEffect(() => {
  ...
  const handleImportJson = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const existingLower = new Set(statuses.map((x) => x.name.trim().toLowerCase()));
    ...
    <div className="w-full" style={{ backgroundColor: "#ecf0f5", minHeight: "100vh", padding: "0" }}>
      ...
      <div className="w-full bg-white" style={{ borderBottom: "1px solid #e0e0e0" }}>
        <div className="flex items-center justify-between px-6 py-3">
          <h1 className="text-lg font-normal" style={{ color: "#666" }}>STATUS OPTION</h1>
        </div>
      </div>
      ...
      <div style={{ padding: "20px" }}>
          ...
          <div className="p-6">
            ...
            <div style={{ marginTop: 14, display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <span style={{ marginLeft: "auto", color: "#888", fontSize: 13 }}>
                Total status options: <strong style={{ color: "#333" }}>{statuses.length}</strong>
              </span>
            </div>
          </div>
        </div>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          ...
                          style={{
                            backgroundColor: status.status === "Active" ? "#337ab7" : "#d9534f",
                            borderRadius: 3,
                          }}
                        >
                          {status.status}
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: 12 }}>
            <div style={{ color: "#666", fontSize: 13 }}>
              ...
              <strong style={{ color: "#333" }}>
                {(filteredAndSortedStatuses.length === 0) ? 0 : (currentPage - 1) * rowsPerPage + 1}
              </strong>{" "}
              to{" "}
              <strong style={{ color: "#333" }}>
                {Math.min(currentPage * rowsPerPage, filteredAndSortedStatuses.length)}
              </strong>{" "}
              of{" "}
              <strong style={{ color: "#333" }}>{filteredAndSortedStatuses.length}</strong> entries
            </div>
        ...
        >
          <div>
            ...
            <strong>{statusToDelete ? statusToDelete.name : ""}</strong>? This action cannot be undone.
          </div>
        </Modal>
      </div>
    </div>
